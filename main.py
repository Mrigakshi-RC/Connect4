...
col7=[7,14,21,28,35,42,49] #each column stores the values assigned to each cell in that very column
...
def assign_to_column(playerno):
    ...
    while True:
        if 1<=col<=7:
            if col==1:
                if check_for_space(col1): #check_for_space checks if col==[] which means column is filled, how? you'll see in the next steps
            ...
            col=col_filled(col)
    #the above loop doesnt end until the chosen column is valid, and when it is, it executes the below statements   
    if playerno==1:
        board[pos]=marker1
    else:
        board[pos]=marker2
